...
if($app->get('/articles/user/([a-zA-Z0-9_]*)')){
    ...
    $sql = "SELECT article_id, title, thumbnail, if (length(content) > 50, concat(substr(content, 1, 50), ' ...'), content) as preview, content, date, type_id FROM article where article_id in (select article_id from article where user_id = '".$params[0]."') order by date desc";
...
if($app->get('/articles/non-user/([a-zA-Z0-9_]*)')){
    $params = $app->getParams();
    $sql = "SELECT article_id, title, thumbnail, preview, content, date, type_id FROM preview where article_id in (select article_id from article where user_id = '".$params[0]."') order by date desc";
...
if ($app->get('/articles/([0-9]*)/user/([a-zA-Z0-9_]*)')) {
    ...
    $sql = "SELECT article_id, title, thumbnail, preview, user_id, type_id FROM preview WHERE type_id =" . strval($params[0]) . " order by date desc;";
    $stmt = $conn->prepare($sql);
...
if ($app->get('/articles/([0-9]*)/search/([ㄱ-ㅎ|가-힣|a-z|A-Z|0-9|%]*)')) {
    ...
    $sql = "SELECT * FROM preview WHERE (title LIKE '%" . strval($urlDecoded) . "%' OR content LIKE '%" . strval($urlDecoded) . "%') and type_id =" . strval($params[0]) . " order by date desc;";
